...
$sql = "update '{$chessGame}' set '{substr($requestPosition,0,1)}'=(select '{substr($piecePosition,0,1)}' where id= '{substr($piecePosition,1,1)}') where id='{substr($requestPosition,1,1)}';"
$result=$mysqli -> query($sql);

//set attacking piece's previous position to 0
$sql = "update '{$chessGame}' set '{substr($piecePosition,0,1)}'=0 where id='{substr($piecePosition,1,1)}';"
$result=$mysqli -> query($sql);
